...
class Solar
{
    public static function solar($usrDate, $lat, $lon) {
        
        $ma_latitude = 50.62925;
        $ma_longitude = 3.057256;

        $time_zone = substr($usrDate->format('O'), -4, 4);
        $time_zone = $time_zone / 100;
        $diff_jour = $usrDate->getTimestamp();


        $heure_secondes = $usrDate->format('H') * 3600;
        $minutes_secondes = $usrDate->format('i') * 60;
        $heure_secondes = $heure_secondes + $minutes_secondes + $usrDate->format('s');
        
        $Time_past_local_midnight = $heure_secondes / 86400;
        
        $Julian_Day = gregoriantojd(03, 22, 2021);
        ...
        else if ($Solar_Elevation_Angle_deg > 5) {
        ...
        else if ($Solar_Elevation_Angle_deg > -0.575) {
        ...
        return  [
                    "Elevation" => $Solar_Elevation_corrected_for_atm_refraction_deg,
                    "Azimuth" => $Solar_Azimuth_Angle_deg_cw_from_N
                ];
